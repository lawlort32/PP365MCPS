...
import { AuthProvider, AccessToken } from './auth/auth-provider.js';
...
export class DataverseClient {
  ...
  constructor(config: DataverseConfig, authProvider: AuthProvider) {
    this.config = config;
  ...
  private async ensureAuthenticated(): Promise<void> {
    if (!this.authToken || Date.now() >= this.authToken.expiresAt) {
      this.authToken = await this.authProvider.getAccessToken(`${this.config.dataverseUrl}/.default`);
    }
  }
  ...
  async getMetadata<T = any>(endpoint: string, params?: Record<string, any>): Promise<T> {
    ...
    await this.ensureAuthenticated();
    if (this.authToken) {
      metadataClient.defaults.headers.Authorization = `Bearer ${this.authToken.accessToken}`;
    }
  ...
  async callAction<T = any>(actionName: string, data?: any): Promise<T> {
    ...
    await this.ensureAuthenticated();
    if (this.authToken) {
      actionClient.defaults.headers.Authorization = `Bearer ${this.authToken.accessToken}`;
    }
