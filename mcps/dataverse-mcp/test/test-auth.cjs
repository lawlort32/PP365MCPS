...
async function main() {
  ...
  const authMode = envVars.DATAVERSE_AUTH_MODE || 'client_secret';
  if (authMode !== 'client_secret') {
    console.log(`⚠️  test-auth.cjs currently validates client_secret flow only. Current mode: ${authMode}`);
    console.log('   Switch DATAVERSE_AUTH_MODE=client_secret for this live auth test.');
    process.exit(0);
  }
