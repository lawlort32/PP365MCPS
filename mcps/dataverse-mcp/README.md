...
```json
{
  "mcpServers": {
    "dataverse": {
      ...
      "env": {
        "DATAVERSE_AUTH_MODE": "client_secret",
...
The server uses a provider-based auth abstraction (`src/auth/*`) selected by `DATAVERSE_AUTH_MODE`.

- `client_secret` (default): current production behavior with client credentials flow.
- `obo`: on-behalf-of token exchange using `DATAVERSE_OBO_ASSERTION`.
- `public_client`: scaffold for interactive public-client login.
- `broker`: scaffold for future WAM/broker integration.

Detailed architecture and wiring notes are documented in:

- `docs/AUTH_MODULE_ARCHITECTURE.md`
